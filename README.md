...
## Overview
This guide is to provide an overview for general emails for SUGM. The goal of this repo is to provide a source of truth and version history for the templates.Due to limitations for email providers, it is best practice to use table layouts for all emails. Below are summaries for email components, layours, and general guidence when developing the emails.
